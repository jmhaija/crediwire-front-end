define([
...
], function(Vue, DictionaryModel, CompanyUserModel, DashboardCollection, dateField) {
    
    var template = [
    '<article class="permissions">',
    '   <aside v-show="!ui.details">',
    '       <section class="identity">',   
    '           <section class="name">{{user.user.name}}</section>',
    '           <section class="tag">{{user.user.email}}</section>',
    '      </section>',
    ...
    '       <button class="primary" v-on:click="save()" v-show="!ui.saving">{{ui.dictionary.connections.save}}</button>',
    '   </aside>',
    '</article>'
    ].join('');
    ...
    var methods = {
        ...
        getDashboardPermissions : function() {
            ...
            um.getDashboardPermissions(this.user.id)
                .then(function(res) {
